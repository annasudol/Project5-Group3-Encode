...
async function initContracts() {
  const lotteryContractFactoryContract = new Lottery__factory(accounts[0]);
  contract = await lotteryContractFactoryContract.deploy(1, BET_PRICE, BET_FEE);
}
...
async function checkState() {
  ...
  console.log(`The lottery is ${state ? 'open' : 'close'}`);
  if (!state) return;
  const currentBlock = await ethers.provider.getBlock('latest');
  const currentBlockDate = new Date(currentBlock.timestamp * 1000);

  console.log(`The last block was minted at ${currentBlockDate.toDateString()}`);

}

async function openBets(duration: string) {
  const currentBlock = await ethers.provider.getBlock('latest');
  // const tx = await contract.openBelts(currentBlock.timestamp + Number(duration));
  // const receipt = await tx.wait()
  // console.log(`belts are open ${receipt.blockHash}`)
}
